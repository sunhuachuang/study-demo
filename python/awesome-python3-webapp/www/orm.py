...
async def create_pool(loop, **kw):
    logging.info('create mysql connect pool...')
    global __pool
...
async def select(sql, args, size=None):
    ...
    with (await __pool) as conn:
        ...
        if size:
            ...
            rs = await cur.fentchall()
        await cur.close()
        logging.info('return {} rows'.format(len(rs)))
        return rs
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    async def findAll(cls):
        rs = await select('{}'.format(cls.__select__))
        obrs = []
        for r in rs:
            obrs.append(cls(**r))
        return obrs
...
class IntegerField(Field):
    def __init__(self, name=None, primary_key=None, default=None, ddl='integer(10)'):
        super().__init__(name, ddl, primary_key, default)
