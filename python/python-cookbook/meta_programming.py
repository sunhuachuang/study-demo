#! /usr/bin/python3
import sys, time, logging, types, weakref, operator
...
def typed_property(name, expected_type):
    ...
    def prop(self, value):
        if not isinstance(value, expected_type):
            raise TypeError('Expected type. {} must a type {}'.format(name, expected_type))
        else:
...
class MyMeta(type):
    def __new__(cls, clsname, bases, clsdict):
        ...
        print('clsdict:', clsdict)   #clsdict: {'__module__': '__main__', '__qualname__': 'B',
