...
prices_derivate = {key:value for key, value in prices.items() if value > 100 } #字典推导
print(prices_derivate)
filter_name = {'a', 'c', 'f'}
prices_filter = {key:value for key, value in prices.items() if key in filter_name } #只有if 没有else
print(prices_filter)
...
rows_by_lname_fname_lambda = sorted(rows, key=lambda r: (r['lname'], r['fname']))
print(rows_by_fname)
print(rows_by_uid)
...
a = Counter(words)
b = Counter(add_words)
print(a+b) #Counter({'eyes': 9, 'the': 8, 'look': 4, 'my': 3, 'into': 3, 'around': 2, "don't": 1, 'not': 1, "you're": 1, 'under': 1})

print(a-b) #Counter({'eyes': 7, 'look': 4, 'my': 3, 'into': 3, 'around': 2, 'the': 2, "don't": 1, 'not': 1, "you're": 1, 'under': 1})
...
print(list(endedupe(d, key=lambda d: (d['x'], d['y'])))) #[{'y': 2, 'x': 1}, {'y': 3, 'x': 1}, {'y': 3, 'x': 2}]
print(list(endedupe(d, key=lambda d: d['x']))) #[{'y': 2, 'x': 1}, {'y': 3, 'x': 2}]
...
print(a.items() & b.items()) #{('c', 34)} keys(), items(), all support, but not values()
...
p.push(Item('aaa'), 4)
p.push(Item('bbb'), 1)
print(p.get()) #[(-5, 1, Item('bar')), (-1, 0, Item('foo')), (-4, 2, Item('aaa')), (-1, 3, Item('bbb'))]
